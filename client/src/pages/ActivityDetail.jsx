...
export default function ActivityDetail() {
  ...
  const loadActivity = async () => {
    setLoading(true);
    try {
      const data = await api.getActivity(slug);
  ...
  if (!activity) return <div className="min-h-screen flex items-center justify-center bg-[#f7f9fb]"><span className="text-slate-500 font-light tracking-widest uppercase text-xs">Activity not found</span></div>;
    ...
    <div className="min-h-screen bg-[#f7f9fb] text-slate-900 page-enter">
      <main className="pt-10 pb-32 flex flex-col items-center px-6">
        ...
        <div className="w-full max-w-6xl mb-10 text-center">
          <Link to="/activities" className="inline-flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-slate-600 hover:text-slate-700 transition-colors mb-4">
            <span className="material-icons text-sm">arrow_back</span> Back to Activities
          </Link>
          ...
          <h2 className="font-serif text-5xl md:text-6xl font-bold tracking-tight text-slate-900">{activity.name}</h2>
        </div>
        ...
        <article className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <div className="md:col-span-7 group relative overflow-hidden rounded-xl shadow-lg">
            <img
              src={activity.images?.length > 0 ? activity.images[currentImageIndex] : activity.coverImage}
              alt={activity.name}
              className="w-full aspect-[4/5] md:aspect-[16/10] object-cover transition-transform duration-500 group-hover:scale-105"
            />
            {activity.images?.length > 1 && (
              <>
                <button onClick={() => setCurrentImageIndex(p => p === 0 ? activity.images.length - 1 : p - 1)} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 text-white p-2 rounded-md transition-all">
                  <span className="material-icons">chevron_left</span>
                </button>
                <button onClick={() => setCurrentImageIndex(p => p === activity.images.length - 1 ? 0 : p + 1)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 text-white p-2 rounded-md transition-all">
            ...
            <div className="absolute bottom-8 left-8">
              <div className="flex items-center gap-2 text-white mb-2">
                ...
                <span className="font-sans text-[10px] uppercase tracking-widest font-bold">{activity.location}, Kashmir</span>
              </div>
              {activity.duration && (
                <span className="bg-white/20 backdrop-blur-sm text-white px-3 py-1 rounded-md font-sans text-[10px] uppercase tracking-widest font-bold flex items-center gap-1 w-fit">
          ...
          <div className="md:col-span-5 flex flex-col gap-8 py-4">
            <header>
              <h3 className="font-serif text-4xl font-bold text-slate-900 mb-4">{activity.name}</h3>
              <p className="text-slate-700 text-lg leading-relaxed">{activity.overview || activity.description}</p>
            </header>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { icon: 'groups', label: 'Group Size', val: activity.groupSize ? `${activity.groupSize.min}–${activity.groupSize.max} pax` : 'Flexible' },
                { icon: 'person', label: 'Min Age', val: activity.ageLimit ? `${activity.ageLimit.min}+ yrs` : 'All ages' },
                { icon: 'wb_sunny', label: 'Best Season', val: activity.bestSeason?.join(', ') || 'Year-round' },
                { icon: 'star', label: 'Rating', val: activity.rating ? `${activity.rating} / 5` : '4.5 / 5' },
              ].map((s, i) => (
            ...
            <div className="mt-auto pt-8 border-t border-slate-200">
              {activity.category && (
                <div className="mb-8">
                  <div className="bg-amber-100 text-amber-800 px-3 py-1 rounded-md inline-block">
                    <span className="font-sans text-[10px] uppercase tracking-widest font-bold">{activity.category}</span>
                  </div>
                </div>
              )}
              <a href={buildWhatsAppUrl(activity.name)} target="_blank" rel="noopener noreferrer"
              ...
              <a href={buildEmailUrl(activity.name)} target="_blank" rel="noopener noreferrer"
        ...
        {/* Safety & Inclusions */}
        {(activity.safetyGuidelines?.length > 0 || activity.inclusions?.length > 0 || activity.requirements?.length > 0) && (
          <section className="w-full max-w-6xl mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            {activity.requirements?.length > 0 && (
              <div className="bg-[#f2f4f6] rounded-xl p-8">
                <span className="font-sans text-[10px] uppercase tracking-widest text-slate-600 font-bold mb-4 block">Requirements</span>
                <ul className="space-y-3">
                  {activity.requirements.map((item, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-slate-600">
                      <span className="material-icons text-slate-500 text-[16px] mt-0.5">info</span>{item}
                    </li>
                  ))}
                </ul>
              </div>
            )}
            {activity.safetyGuidelines?.length > 0 && (
              <div className="bg-[#f2f4f6] rounded-xl p-8">
                <span className="font-sans text-[10px] uppercase tracking-widest text-slate-600 font-bold mb-4 block">Safety Guidelines</span>
                <ul className="space-y-3">
                  {activity.safetyGuidelines.map((item, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-slate-600">
                      <span className="material-icons text-red-400 text-[16px] mt-0.5">warning</span>{item}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-lg p-8 shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-serif text-2xl font-bold text-slate-900">Book Activity</h2>
            ...
            <form onSubmit={handleEnquiry} className="space-y-4">
              ...
              <div>
                <label className="font-sans text-[10px] uppercase tracking-widest font-bold text-slate-600 block mb-1">Preferred Date</label>
              ...
              <div>
                <label className="font-sans text-[10px] uppercase tracking-widest font-bold text-slate-600 block mb-1">Number of People</label>
