...
export default function HouseboatDetail() {
  ...
  const [houseboat, setHouseboat] = useState(null);
  ...
  const loadHouseboat = async () => {
    setLoading(true);
    try {
      const data = await api.getHouseboat(slug);
      if (data) {
        setHouseboat(data);
      } else {
        setHouseboat(null);
  ...
  if (!houseboat) return <div className="min-h-screen flex items-center justify-center bg-[#f7f9fb]"><span className="text-slate-500 font-light tracking-widest uppercase text-xs">Houseboat not found</span></div>;

  const categoryLabels = { 'deluxe': 'Deluxe', 'super-deluxe': 'Super Deluxe', 'luxury': 'Luxury', 'standard': 'Standard' };
    ...
    <div className="min-h-screen bg-[#f7f9fb] text-slate-900 page-enter">
      <main className="pt-10 pb-32 flex flex-col items-center px-6">
        ...
        <div className="w-full max-w-6xl mb-10 text-center">
          <Link to="/houseboats" className="inline-flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-slate-600 hover:text-slate-700 transition-colors mb-4">
            <span className="material-icons text-sm">arrow_back</span> Back to Houseboats
          </Link>
          <span className="font-sans uppercase tracking-widest text-[10px] text-slate-600 font-bold mb-2 block">
            {categoryLabels[houseboat.category] || houseboat.category} · {houseboat.lake?.replace('-', ' ').toUpperCase()}
          </span>
          <h2 className="font-serif text-5xl md:text-6xl font-bold tracking-tight text-slate-900">{houseboat.name}</h2>
        </div>
        ...
        <article className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <div className="md:col-span-7 group relative overflow-hidden rounded-xl shadow-lg">
            <img
              src={houseboat.images?.length > 0 ? houseboat.images[currentImageIndex] : houseboat.coverImage}
              alt={houseboat.name}
              className="w-full aspect-[4/5] md:aspect-[16/10] object-cover transition-transform duration-500 group-hover:scale-105"
            />
            {houseboat.images?.length > 1 && (
              <>
                <button onClick={() => setCurrentImageIndex(p => p === 0 ? houseboat.images.length - 1 : p - 1)} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 text-white p-2 rounded-md transition-all">
                  <span className="material-icons">chevron_left</span>
                </button>
                <button onClick={() => setCurrentImageIndex(p => p === houseboat.images.length - 1 ? 0 : p + 1)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 text-white p-2 rounded-md transition-all">
            ...
            <div className="absolute bottom-8 left-8">
              <div className="flex items-center gap-2 text-white mb-2">
                ...
                <span className="font-sans text-[10px] uppercase tracking-widest font-bold">{houseboat.lake?.replace('-', ' ')}, Kashmir</span>
              </div>
              <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-md font-sans text-[10px] uppercase tracking-widest font-bold ${houseboat.availability === 'available' ? 'bg-green-100 text-green-800' : houseboat.availability === 'limited' ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800'}`}>
                {houseboat.availability === 'available' ? '✓ Available' : houseboat.availability === 'limited' ? '⚠ Limited' : '✗ Booked'}
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-5 flex flex-col gap-8 py-4">
            <header>
              <h3 className="font-serif text-4xl font-bold text-slate-900 mb-4">{houseboat.name}</h3>
              <p className="text-slate-700 text-lg leading-relaxed">{houseboat.overview || houseboat.description}</p>
            </header>

            {/* Stats */}
            <div>
              <span className="font-sans text-[10px] uppercase tracking-widest text-slate-600 font-bold mb-4 block">Houseboat Details</span>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { icon: 'bed', label: 'Bedrooms', val: houseboat.rooms?.bedrooms || '—' },
                  { icon: 'bathroom', label: 'Bathrooms', val: houseboat.rooms?.bathrooms || 1 },
                  { icon: 'groups', label: 'Capacity', val: `${houseboat.capacity?.guests || '—'} guests` },
                  { icon: 'weekend', label: 'Living Rooms', val: houseboat.rooms?.livingRooms || 1 },
                ].map((s, i) => (
              ...
              <div className="flex items-baseline justify-between mb-8">
                <div>
                  ...
                  <div className="price-display">
                    <span className="price-amount">₹{houseboat.pricing?.perNight?.toLocaleString()}</span>
                    <span className="price-unit">/ night</span>
                  </div>
                </div>
                <div className="text-right text-[10px] font-sans font-bold text-slate-600 uppercase tracking-widest">
                  <div>In {houseboat.checkIn || '12 PM'}</div>
                  <div>Out {houseboat.checkOut || '10 AM'}</div>
                </div>
              </div>
              <a href={buildWhatsAppUrl(houseboat.name)} target="_blank" rel="noopener noreferrer"
        ...
        {/* Highlights / Features */}
        {houseboat.highlights?.length > 0 && (
          <section className="w-full max-w-6xl mt-16 bg-[#f2f4f6] rounded-xl p-12">
            <span className="font-sans text-[10px] uppercase tracking-widest text-slate-600 font-bold mb-6 block">Life on the Lake</span>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {houseboat.highlights.slice(0, 3).map((h, i) => (
                <div key={i} className="flex flex-col gap-3">
                  <span className="material-icons text-slate-900">sailing</span>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-lg p-8 shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-serif text-2xl font-bold text-slate-900">Book Houseboat</h2>
