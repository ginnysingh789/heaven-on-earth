        ...
        <article className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <div className="md:col-span-5 flex flex-col gap-8 py-4">
            ...
            <div className="mt-auto pt-8 border-t border-slate-200">
              <span className="price-label mb-4">Pricing</span>
              <div className="space-y-3 mb-8 bg-green-50 border border-green-200 rounded-xl p-4">
                {rental.pricing?.perDay && (
                  <div className="flex items-baseline justify-between">
                    <span className="text-slate-600 text-sm font-medium">Per Day</span>
                    <span className="font-serif text-2xl font-bold text-green-700">₹{rental.pricing.perDay.toLocaleString()}</span>
                  </div>
                )}
                {rental.pricing?.perKm && (
                  <div className="flex items-baseline justify-between">
                    <span className="text-slate-600 text-sm font-medium">Per Km</span>
                    <span className="font-serif text-xl font-bold text-green-700">₹{rental.pricing.perKm.toLocaleString()}</span>
                  </div>
                )}
                {rental.pricing?.perTrip && (
                  <div className="flex items-baseline justify-between">
                    <span className="text-slate-600 text-sm font-medium">Per Trip</span>
                    <span className="font-serif text-xl font-bold text-green-700">₹{rental.pricing.perTrip.toLocaleString()}</span>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-4">
              {rental.routes.map((route, i) => (
                <div key={i} className="bg-white rounded-md px-6 py-4 flex items-center justify-between">
                  ...
                  <span className="font-serif text-xl font-bold text-slate-900">₹{route.price?.toLocaleString()}</span>
                </div>
              ))}
            </div>
          </section>
